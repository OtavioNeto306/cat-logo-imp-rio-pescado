...
const CategoryManager: React.FC<{
    ...
    // Fix: Updated the type of the 'category' parameter for addCategory to match the `useProducts` hook.
    // The `isActive` property is set internally by the hook.
    addCategory: (category: { name: string; imageUrl: string }) => boolean | Promise<boolean>;
    updateCategory: (slug: string, data: { name: string; imageUrl: string }) => boolean | Promise<boolean>;
    deleteCategory: (slug: string) => void | Promise<boolean>;
    toggleCategoryActive: (slug: string) => void | Promise<boolean>;
    ...
    const handleSubmit = async (data: { name: string; imageUrl: string }) => {
        let success = false;
        if (editingCategory) {
            success = await updateCategory(editingCategory.slug, data);
        } else {
            success = await addCategory({ name: data.name, imageUrl: data.imageUrl });
        }
    ...
    const handleToggleActive = async (category: Category) => {
        if (window.confirm(`Tem certeza que deseja ${category.isActive ? 'INATIVAR' : 'ATIVAR'} a categoria "${category.name}" e todos os produtos associados?`)) {
            await toggleCategoryActive(category.slug);
        }
    };
...
const AdminPage: React.FC = () => {
  const { allProducts, allCategories, addProduct, updateProduct, deleteProduct, addCategory, updateCategory, deleteCategory, toggleCategoryActive, isLoading, error } = useProducts();
